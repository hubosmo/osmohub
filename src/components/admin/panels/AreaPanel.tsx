...
import { atualizarAreaContent, deletarAreaContent, uploadCapaArea } from "@/lib/actions/content";
import { AdminSaveForm } from "@/components/admin/AdminSaveForm";
import { CapaUploadSection } from "@/components/admin/panels/CapaUploadSection";
...
type AreaData = {
  ...
  publicado: boolean;
  topicos: { id: string }[];
  disciplina: { id: string; nome: string; curso: { id: string; nome: string } };
};

export function AreaPanel({ data }: { data: AreaData }) {
  const saveAction = atualizarAreaContent.bind(null, data.id);
  const deleteAction = deletarAreaContent.bind(null, data.id, data.disciplina.id);
  ...
  return (
    <div className="max-w-xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            Unidad · <span style={{ color: "var(--accent)" }}>{data.disciplina.nome}</span>
          </p>
        ...
        <span className="ml-auto text-xs px-2.5 py-1 rounded-full"
          ...
          {data.topicos.length} {data.topicos.length === 1 ? "tópico" : "tópicos"}
        </span>
      </div>

      <div className="rounded-xl p-5" style={{ backgroundColor: "var(--bg-surface)", border: "1px solid var(--border)" }}>
        <AdminSaveForm id="form-area-panel" action={saveAction} className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between mt-4 pt-4" style={{ borderTop: "1px solid var(--border)" }}>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="publicado" form="form-area-panel"
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" form="form-area-panel"
      ...
      <CapaUploadSection
        entityId={data.id}
        capaUrlDark={data.capa_url}
        capaUrlLight={data.capa_url_light}
        uploadAction={uploadCapaArea}
        aspectRatio="1/1"
      />
    </div>
  );
